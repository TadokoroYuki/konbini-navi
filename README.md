...
| 領域 | 技術 |
|------|------|
| モバイル | Expo SDK 54 + Expo Router v6 |
| バックエンド | Go 1.24 マイクロサービス (4サービス, gRPC通信) |
| DB | PostgreSQL 15 |
| インフラ | AWS CDK (VPC + ECR + Cognito) |
...
│   ├── api/             # Go マイクロサービス (products, records, nutrition, recommendations)
...
# Docker Compose でAPI + PostgreSQL を起動
docker compose up
